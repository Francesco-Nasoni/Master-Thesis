...
The molasses is performed with a detuning different from that used for the MOT phase, it is changed with a ramp of 20 steps with a total duration of 5~ms. The ramp stars 5~$\mu$s after the shim coils are changed to their molasses value. 
...
%* ##########################################################
In the analysis of the ODT, two main figures of merits are taking into account, namely the loading efficiency $\eta$ and the intensity integral of the ODT peak, that is directly porportional to the number of captured atoms. They are measured by comparing absorption images acquired with the ODT either present or absent during the MOT and molasses stages. Since only a small fraction of the atoms is expected to be captured by the ODT (order of $1/1000$), the analysis relies on differential imaging and careful background normalization to isolate the signal of the trapped atoms from fluctuations in the total atom number. The complete analysis procedure is summarized below.

\begin{enumerate}
    \item Two identical experimental sequences of MOT and molasses are consecutively performed with the same parameters. In one sequence, the ODT is kept on throughout the entire cycle starting from the MOT loading phase, whereas in the other the ODT remains off. After a TOF of 30~ms, absorption images of the atomic distribution denoted by $I_\text{on}$ and $I_\text{off}$ are taken with the Manta camera.
    During the TOF the atom not trapped in the ODT expands enhancing the visibility of those confined in the ODT. Each pair f measurements is repeated five times
